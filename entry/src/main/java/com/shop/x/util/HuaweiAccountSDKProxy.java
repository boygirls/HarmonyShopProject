...
public final class HuaweiAccountSDKProxy {
    ...
    /**
     * 先静默登录，如果静默登录成功，则从成功回调方法中获取AuthAccount用户信息，此时用户静默登录成功，获取到用户信息（包括Id Token），登录流程结束；
     * 如果静默登录失败，则在失败回调方法中，显示调用前台登录授权signIn接口，进行登录授权。
     * 调用前台登录授权signIn接口：
     * 如果前台登录授权接口执行成功，在前台登录授权signIn接口的成功回调方法中获取AuthAccount用户信息（包括Id Token），登录流程结束。
    ...
    public void signOut() {
        ...
        task.addOnFailureListener(new OnFailureListener() {
            ...
            public void onFailure(Exception e) {
                ...
                if (e instanceof ApiException) {
                    ApiException apiException = (ApiException) e;
                    // 华为帐号登出失败，status code标识了失败的原因，请参考API中的错误码参考了解详细错误原因
                    apiException.getStatusCode();
